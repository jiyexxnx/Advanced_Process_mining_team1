...
def embedding_model(params):
    ...
    if LPMs:
        if Only_LPMs:
            ...
            result = model.fit(train_lpms, train_y,
            ...
            start_time = time.time()
            result = model.fit([train_x, train_lpms], train_y,
                               batch_size=params['batch_size'],
                               epochs=1000,
                               validation_split=0.2,
                               callbacks=early_stopping)
...
parser.add_argument("--raw_data",
                    type=str,
                    default="./datasets/Production_Trunc23_DiscriminativeLPMs_Agg.csv",
                    help="address to raw dataset")

parser.add_argument("--out_name",
                    type=str,
                    default="production_discr",
...
if not os.path.exists(f"{datasets_dir}/{dataset_name}"):
    ...
    train_x, train_y, train_lpms, test_x, test_y, test_lpms = data_manager.encode_prefixes(LPMs, 0.8, sample=args.sample)
...
output_file = output_datasets_address + args.results
if LPMs:
    if Only_LPMs:
